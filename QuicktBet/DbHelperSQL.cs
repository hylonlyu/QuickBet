...
namespace EatZD
{
    public class DbHelperSQL : IDisposable
    {
        ...
        private string connectionString = @"server=.;database=tt4;User Id=kanfei11;Password=#KanFei#";
        ...
        public DbHelperSQL()
        {
            //using (StreamReader sr = new StreamReader("EntityFramework.Server.dll"))
            //{
            //    CryptoHelper cry = new CryptoHelper();
            //    string conn = cry.GetDecryptedValue(sr.ReadToEnd());
            //    string[] tmp = conn.Split("|".ToCharArray());
            //    if (tmp.Length == 4)
            //    {
            //        connectionString = $"server={tmp[0]};database={tmp[1]};User Id={tmp[2]};Password={tmp[3]}";
            //    }
            //}
        }
